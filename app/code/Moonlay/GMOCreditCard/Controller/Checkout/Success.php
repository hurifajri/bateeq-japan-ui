...
class Success extends AbstractAction implements HttpPostActionInterface
{
    public function execute()
    {
        ...
        if (!$orderId) {
            $this->getLogger()->debug("GMO Credit Card returned a null order id. This may indicate an issue with the GMO Credit Card gateway.");
        ...
        if (!$isValidPayment) {
            $this->getLogger()->debug("GMO Credit Card returned response total price was does not match the order id: $orderId");
        ...
        if (empty($ErrCode) && empty($ErrInfo)) {
            ...
            $order->setState($orderState)
                ...
                ->addStatusHistoryComment("GMO Credit Card authorisation success. Transaction #$orderId")
            ...
            $this->getCheckoutHelper()->cancelCurrentOrder("Order #$orderId was rejected by GMO Credit Card. Transaction #$transactionId.");
            $this->getCheckoutHelper()->restoreQuote(); //restore cart
