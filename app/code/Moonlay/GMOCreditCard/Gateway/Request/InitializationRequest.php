...
class InitializationRequest implements BuilderInterface
{
    ...
    private function validateQuote(OrderAdapter $order) {
        ...
        if($total < 1) {
            $this->_session->setErrorMessage(__("GMO Credit Card doesn't support purchases less than ¥1."));
            return false;
        }
        ...
        if (!in_array($order->getBillingAddress()->getCountryId(), $allowedCountriesArray)) {
            ...
            $this->_session->setErrorMessage(__('Orders from this country are not supported by GMO Credit Card. Please select a different payment option.'));
            return false;
        }
        ...
        if (!in_array($order->getShippingAddress()->getCountryId(), $allowedCountriesArray)) {
            $this->_session->setErrorMessage(__('Orders shipped to this country are not supported by GMO Credit Card. Please select a different payment option.'));
            return false;
        }
